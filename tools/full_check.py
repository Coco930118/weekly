...
def main(path):
    ...
    NUM = (r'((二|三|四|五|六|七|八|九|十|[2-9]|[0-9]{2,})(日|回|人|件|度目|日目|つ)'
           ...
           r'|(二|三|四|五|六|七|八|九|[2-9]|[0-9]{2,})分'
           r'|十分(?!よ|です|だ(?!け)))')
    # 伝聞マーカー。裸の「って」は動詞の活用（思ってた・黙っている・持って。）に当たり、
    # 3週68本でこの歯止めを無効化していた（2026-08-29 Wチェック実測）。
    # 引用を閉じる形・人/話/聞くに続く形だけを伝聞として扱う
    DEN = (r'(」って|って人|って話|って聞|って言わ|だって|んだって|らしい|みたい|そう。|'
           r'人がいた|人がいる|という人|聞いた|相談があった|と話してくれた)')
    numless = []
    for p in posts:
