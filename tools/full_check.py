...
ZAN_FROM = '2026-09-15'
# 「戻す」の比喩的な用法（2026-09-21 Coco決定・恒久ルール）。条文の正典は
# rules/posts.md「抽象語を使わない」の置き換え表。ここに理由と置き換え先を書かない。
# **機械は語を拾うだけ。比喩か具体かは目視で決める**（「渡す」と同じ扱い）。
# 「戻り方」＝処方の名前は Coco の指定で例外なので、機械の側でも外す。
# ZAN と同じく BANNED には入れない——日付を見ないので過去記事まで一斉に鳴る。
# 開始日が 9/22 なのは、2026-09-21 Coco決定のイレギュラー対応（9/22週の35投稿と
# note 全記事をさかのぼって直す）に合わせたため。
MODOSHI_RE = re.compile(r'戻(?!り方)[るらりれっさしすせそ]')
MODOSHI_FROM = '2026-09-22'
# 「置かれる／置いてある」＝状態の言い方（2026-09-24 Coco決定・恒久ルール）。
# 条文の正典は rules/posts.md「抽象語を使わない」の置き換え表。
# **動作の「置く」は対象にしない**——処方の動作語で、下の TOME（note案内が
# 「返信に置いた」で閉じているか）が要求している側。受け身・存在の形だけを拾う。
OKU_RE = re.compile(r'置か[れせ]|置いてあ')
OKU_FROM = '2026-09-22'
...
def main(path):
    ...
    for p in posts:
        ...
        if p['date'] >= MODOSHI_FROM:
            mo = sorted(set(MODOSHI_RE.findall(re.sub(r'「[^」]*」', '', t))))
            if mo: ng(p['id'], '「戻す」（比喩なら置き換える。具体の出来事ならそのまま・目視）', mo)
        if p['date'] >= OKU_FROM:
            ok = sorted(set(OKU_RE.findall(re.sub(r'「[^」]*」', '', t))))
            if ok: ng(p['id'], '「置かれる／置いてある」（状態の言い方。起きたことで書く）', ok)
    ...
    for p in X:
        ...
        if p['date'] >= MODOSHI_FROM:
            mo = sorted(set(MODOSHI_RE.findall(re.sub(r'「[^」]*」', '', flat))))
            if mo: ng(p['id'], 'X短文に「戻す」（比喩なら置き換える・目視）', mo)
    ...
    ACT = (r'(てみ|てみない|今日ひとつ|今夜ひとつ|明日ひとつ|今週ひとつ|'
           r'はどう？|でいいよ|で十分|'
           r'そうしてみ)')
    # 2026-09-19：**裸の動詞語幹の2群を外した**（決め／数え／書い／降り、入れて／聞いて／
    # 置いて／添えて／立って／外して／切って／渡して／返して／止めて）。
    # 語幹は情景の描写に当たり続けていた——実測でこの日だけで4件：
    #   9/15週 th_18「誰が決めたわけでもない時間に」・th_21「決めながら歩く日と、何も決めずに歩く日」
    #   9/22週 th_15「前の日に全部外しておく」「起きる時間も決めない」
    # **佇まい枠3本が1本に、9/22週は2本が1本に見えていた**（目安2〜3を割る）。
    # 2026-08-29 に「〜てい〜」の除外を足したのも同じ事故で、語幹のままでは繰り返す。
    # 較正（2026-09-19・8月以降の全週）：佇まい素材を持つThreads 10本のうち、
    # 上の強い処方語に当たるのは**0本**。10本とも実物が佇まい枠で、誤検出ゼロ・見落としゼロ
