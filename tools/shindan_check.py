...
X_FOUR = '四つとも、間違いではない。'
# 間合い診断への接続（rules/shindan.md「間合い診断への接続」が正典）
# 2026-08-30：旧々版「…その場面ではなく、いつもの判断／癖のほう。」から、
#   型の名前を言い切る形へ。読者が知りたいのは「わたしは何型？」だから。
# 2026-09-01：接続から数字（「あと7問」「残り7問」）を外し、主語を読者に返した。
#   語順の事故だった——目に先に入るのが「あと7問」で、着手コストを下げるはずの数字が
#   残タスクの告知として読まれていた。所要時間は「▼ 8問 / 約1分」1箇所だけが持つ。
# **どちらの回もツールが旧文言を持ったまま残り、正典どおりに書いた本文が落ちた。**
# 接続を変えるときは、この3定数を同じコミットで直す。
...
TH_BRIDGE = 'さっきの4択が1問目。次に出るのは、答えじゃなくて、あなたの型の名前。\n溶ける人／秘める人／尽くす人／守る人。'   # 2行で1つ。正典は rules/shindan.md「reply_1 は接続の2行だけ差し替える」（2026-09-05 型名4つを2行目に追加・9/8週から）
