...
合言葉は `rules/check.md` の表のとおり：
- **「恒久ルール通りかチェックして」** ＝ 直近に作業した対象だけ
- **「全て恒久ルール通りかチェックして」** ＝ その週の全媒体（35投稿＋診断14本＋note9本）

```
全て恒久ルール通りかチェックして。対象週：YYYY-MM-DD〜YYYY-MM-DD

■ 回すもの（rules/check.md の「「全て〜」のときに回す一覧」に従う）
1. python3 tools/full_check.py    posts/week_*.json
2. python3 tools/shindan_check.py posts/shindan_*_YYYY-MM-DD.json
3. python3 tools/note_check.py    --week YYYY-MM-DD_YYYY-MM-DD
4. 3つのツール末尾の【判断チェック】を、媒体ごとに通しで埋める
5. 媒体をまたいで見る（12:00のグローアップは媒体別なので、ここでしか見えない）
