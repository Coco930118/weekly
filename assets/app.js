...
- 診断内・同週の診断投稿を通して、重複は「同じ単語」だけで判定しない。「〜かもしれない」「〜と思う」「〜なんだよね」のような語尾・文型・言い回しの型が近い間隔で続いていないかも見る。意味が同じでも、自然な日本語のリズムとして単調なら重複として調整する。
- ただし語尾を変えること自体を目的に不自然な言い換えをしない。Cocoらしい自然さと意味の精度を優先し、反復に意図的なリズムや強調の機能がある場合は残す。
- ただし素材にない診断結果・心理・エピソードは作らない。既存のaxis_mapや診断URL等のデータは勝手に変更しない。

【Final Editorの最上位3軸｜最終合否はこの3点で決める】
① ファン化につながるか
単に「いいことを言っている」で終わらず、「この人の考え方が好き」「また読みたい」が残るか。Coco固有の視点・実体験・人間味が感じられるか。Xでは仕事・組織に対する「この視点は使える」、Threadsでは人間関係に対する「この感覚、わかる」が入口になっているか。

② 深掘りnote記事への布石になっているか
投稿だけで全部を説明し切らず、「この考え方をもう少し知りたい」が自然に残るか。無理なnote誘導ではなく、SNSでは問い・気づき・視点を渡し、noteで構造・判断軸・再現方法を深掘りできる余白があるか。note導線が付いていない投稿でも、この役割を満たせるかで判断する。

③ Coco Methodologyのブランドに合っているか
「関係の温度と距離を整える技術」につながっているか。押しつけず、曖昧にもせず、感情・存在を否定せずに選択とその先を見せ、最後の決定権を本人へ返しているか。Cocoの静かな強さ・実体験の手触りを一般論へ薄めていないか。

この3軸は新しい投稿パーツではなく、既存編集全体を評価する最上位基準。細かい改善点は原則この3軸のどこかに吸収し、4軸目・5軸目を増やさない。
3点すべてOKの完成稿だけを最終的な公開OKとする。

【制作上の確認項目｜3軸を満たすために使う。最終合否軸ではない】
1. 一読で、何を言いたい投稿か分かるか。
