...
function buildFinalEditorPrompt(post) {
  ...
  const requiredChoiceInstruction = '【今回の必須表示】①A｜いつもの構成 と ②B｜澄んだ短文 の2案を、従来の「選択できる2案」の横並び表示で提示してください。最初のフックを左右で比較できることを最優先にし、各案はその場で文章編集できる形式にしてください。';
...
表示は、これまで使っていた「選択できる2案」の横並びを最優先で維持する。AとBの最初のフックを左右で比較できることを目的とし、各案はその場で文章を編集できる形式にする。
...
- Final Editorの選択肢は全対象でA/Bの2案に固定する。
