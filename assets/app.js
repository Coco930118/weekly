...
【Xの声・温度｜正典を1か所だけ参照】
- X本文の声・配分・着地・温度判定は、main の rules/posts.md「Xの声＝凛とした軍師」が唯一の正典。ここには定義・比率・判定文を複製しない。X本文を編集する前に、その節を実物参照して従う。
- X短文は別枠。main の rules/posts.md「X短文（観察）」を正典として参照し、X本文の温度基準を短文へ持ち込まない。
- Threadsの声は既存正典に従い、この節では再定義しない。

【X本文・返信を動かしたときのnote整合｜運用だけをここに置く】
- 約束と回収・promise_map の意味と判定は、main の rules/check.md「約束と回収を突き合わせる」および rules/note.md を正典として実物参照する。ここには定義を書き写さない。
- Final EditorがX本文または既存返信を編集した場合だけ、「どこを／どう変えたか」の変更一覧を残し、紐づくnoteを取得できる場合は上記正典に沿って影響を確認する。
- Xが編集対象なら、紐づくnoteは参照のみ。勝手に同時編集しない。取得できなければ「未確認」と明示し、推測で「影響なし」としない。
- 温度だけの修正では、promise_mapや約束の意味を動かさない範囲を優先する。
