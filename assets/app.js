...
- X短文は直後のXへの入口。X短文を編集するときは直後のXを参照し、答えを先に言い切らず、弱すぎず、同じ内容の反復にもならないようにする。
- Xを編集するときは、入口になっている直前のX短文と、深掘り先のnoteを参照する。X短文で生まれた関心を受け取り、noteで渡す判断原理・再現方法を食わないところまでを担当する。
...
診断本文と解説コメントを1セットで編集する（X診断=comment、Threads診断=reply_1）。解説コメントはA/B/C/D表記だけで選択肢文言を再掲せず、各解説・見るポイント／一手・結び・診断接続まで本文と整合させる。X/Threadsで読み切れる量と視認性に整える。
...
- Xは折り畳み前の判断を守り、Threadsは余白と自然な温度を優先する。
