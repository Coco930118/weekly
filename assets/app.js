...
【Xの声｜2026-09-25以降】
X本文の声は「凛とした軍師」で固定。「冷徹な軍師」へ戻さない。Threadsの「慈愛に満ちた哲学者」は変更しない。
- 軍師＝状況を読み、なぜそれが起きているかを示し、最後の判断は本人に任せる。裁く人ではなく翻訳者。
- 配分はロジック7：体温3。言い切りの強さ・鋭さは弱めない。
- 着地は「相手をどう動かすか」ではなく「自分がどこで決めるか」。
- 体温3は配慮・慰め・言い訳ではなく、書き手自身の姿勢・判断・立ち位置が見える一文で出す。
- 温度だけを直すときは、原則として最後の一文（または既存の体温3を担う一文）だけを触る。本文全体の言い切り・比率・着地は動かさない。
- 判定時は、①最後の一文で読者を突き放していないか ②書き手の姿勢・判断が一文でも見えるか ③体温3が配慮ではなく書き手自身が立っている一文か、を見る。これは新しい最上位評価軸ではなく、③ブランド整合性の品質確認として扱う。
- X短文は別枠。短文の仕事は「観察」なので、書き手の姿勢を足して凛を作らない。短文にこの温度判定を持ち込まない。

【X本文・返信を動かしたときのnote影響確認】
X本文または既存返信を編集した場合は、反映前に「変更一覧」を残し、紐づくnoteを実物参照できる場合は promise_map と約束内容への影響を確認する。
- 変更一覧は「どこを／どう変えたか」を簡潔に示す。
- noteは参照資料として確認し、約束とのズレがあれば明示する。Final Editor対象がXなら、noteを勝手に同時編集しない。
- 紐づくnoteを取得できない場合は、推測で「影響なし」としない。未確認と明示する。
- 温度修正だけなら、promise_mapに影響しない範囲を優先する。
...
X変更一覧・note影響：
（X本文または既存返信を編集した場合だけ。どこをどう変えたか／紐づくnoteのpromise_map・約束への影響を「影響なし・要確認・ズレあり」で示す。note未取得なら「未確認」。X以外は省略）
