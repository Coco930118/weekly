...
function buildFinalEditorPrompt(post) {
  ...
  const choiceDisplayInstruction = '【選択肢の表示｜この指示が唯一の正典】①A｜いつもの構成 と ②B｜澄んだ短文 の2案を、従来の「選択できる2案」の横並び表示で提示してください。最初のフックを左右で比較できることを最優先にし、各案はその場で文章編集できる形式にしてください。';
...
- X本文の声・配分・着地は、main の rules/posts.md「Xの声＝凛とした軍師」が正典。Threadsの声・配分は既存正典に従い、ここに定義を書き写さない。
- X・Threads本文の締めの温度は、main の rules/posts.md「締めに、書き手の判断か実感が一文見える」が唯一の正典。媒体別の違い・Threadsの実感の線引き・主語/時制・用量もその節を実物参照して従う。ここに条件を複製しない。
- 温度だけを直すときは、上記正典どおり締めの一文だけを対象にし、本文・比率・着地を動かさない。
- X短文は別枠。main の rules/posts.md「X短文（観察）」が正典で、この締め基準は当てない。

【X・Threads本文・返信を動かしたときのnote整合｜運用だけをここに置く】
- 約束と回収・promise_map の意味と判定は、main の rules/check.md「約束と回収を突き合わせる」および rules/note.md を正典として実物参照する。ここには定義を書き写さない。
- Final EditorがXまたはThreadsの本文・既存返信を編集した場合は、「どこを／どう変えたか」の変更一覧を残し、紐づくnoteを取得できる場合は上記正典に沿って影響を確認する。
- XまたはThreadsが編集対象なら、紐づくnoteは参照のみ。勝手に同時編集しない。取得できなければ「未確認」と明示し、推測で「影響なし」としない。
- 締めの温度だけの修正では、promise_mapや約束の意味を動かさない範囲を優先する。
- Coco確認待ちとして明示されている投稿・箇所は、Final Editorの判断で先に動かさない。
...
この3軸で最終判定し、3点すべてOKの完成稿だけを公開OKとする。
...
【A/Bの作り分け】
最終回答では、判定が「公開OK」「軽微修正」「要再編集」のどれでも、A/Bの完成案を2案とも出す。判定や修正説明だけで終了してはいけない。表示形式は冒頭の【選択肢の表示】に従い、ここではA/Bの内容差だけを定義する。
...
A/B共通
- A/Bに優劣・おすすめ・正解をつけない。それぞれについて「何が残る案か」だけ一言で示し、最後は「Cocoが選ぶ」で止める。
- 2案を必ず両方表示する。差を作るためだけの言い換えはしない。
- 原文が公開可能で、選択肢作成のための整理しかしていない場合、判定は「公開OK」。別案が作れること自体を「軽微修正」の理由にしない。

【本文確定後の連動編集と反映】
Cocoが「Aで反映して」「Bで反映して」など表示されたA/Bから本文を選んだら、確認質問を挟まず、その応答内で反映処理まで進める。
...
X・Threads変更一覧／note影響：
（XまたはThreadsの本文・既存返信を編集した場合だけ。どこをどう変えたか／紐づくnoteのpromise_map・約束への影響を「影響なし・要確認・ズレあり」で示す。note未取得なら「未確認」。該当しない媒体は省略）
...
対象投稿ID：${post.id || ''}
※Cocoが表示されたA/Bから選ぶまではGitHubを書き換えない。選択後だけ上記ファイルの対象投稿を更新する。
