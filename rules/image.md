...
### noteの型マガジン表紙（1280×670・2026-08-30 Coco決定・恒久ルール）

**診断のカードと同じ絵を流用しない。仕事が違う。**

| | 見る人 | 主役 | 締め |
|---|---|---|---|
| **結果シェアOG**（1200×630） | まだ診断していない人 | 型名 | **あなたは？ 8問 / 約1分** |
| **型マガジン表紙**（1280×670） | **もう会員か、販売ページにいる人** | **マガジン名「◯◯人へ」** | **診断のCTAを入れない** |

- **「あなたは？ 8問 / 約1分」を表紙に置かない。** 有料マガジンの表紙に診断のCTAがあると、**払った人にもう一度入口を見せる**ことになる
- **主役はマガジン名（`rules/note.md`）**。「溶ける人」ではなく**「溶ける人へ」**——マガジンは型そのものではなく、**その型に宛てた棚**
- **アイブロウはプラン名**（『距離の整え方 —— 恋愛と関係』『温度の整え方 —— 組織と仕事』）。どちらのプランの棚かが一覧で分かる
- **一行は褒めの側**（結果シェアと同じ。見せて恥ずかしくないもの）
- **8枚。中央「保つ人」は作らない**——中央は記事の宛先にならないので型マガジンが無い（`rules/note.md`「中央『保つ人』は割り当てない」）
- 生成は `tools/ogmag.py`

### 汎用カード（トップ・ドメイン）も型名を出す（2026-08-30 Coco決定・恒久ルール）

**日別・型別のほかに、差し替わらない汎用カードが4枚ある。ここが最初に古くなる**——ページの言葉を直しても、絵は誰も見に行かないため。

| ファイル | 使うページ | 中身 | 生成 |
|---|---|---|---|
| `og/og-root.png` | `/`・`/voices/*` | h1 ＋ **恋愛4型／仕事4型** ＋ 3匹 ＋ 8問 / 約1分 | `tools/ogroot.py` |
| `og/kankei-og.png` | `/kankei` | 入口カードと同じ体裁。見出しは h1、型名は恋愛4型 | `tools/oggen.py` |
| `og/soshiki-og.png` | `/soshiki` | 同上・組織4型 | 同上 |
| `og/type-og.png` | `/type`（**別診断**・未公開） | 3匹の型診断のもの。**間合い診断の管轄外** | — |

- **トップページの言葉を変えたら、必ず `og-root` も焼き直す。** 2026-08-30 の時点で、旧h1「関係の温度と距離を整える技術」がカードに残り、**型名は1つも出ていなかった**
- **`kankei-og` / `soshiki-og` に「あなたは、どの子？」を書かない**（廃止。**出るのは型であってキャラではない**）
- **`type-og` は `/type` のもので、間合い診断ではない。** 言葉を weekly で新規に作らない。**正典にある語への置き換え**（「配合」→「全員集合」）だけ実施する
